// PoC-C: der eigentliche Forza-Fall - WGC-FENSTER-Capture (CreateForWindow) statt
// Monitor -> NVENC -> H.264. Beweist, dass die Fenster-Aufnahme, die im C#-Helfer
// den CPU-Readback-Flaschenhals hatte, in C++ zero-copy laeuft. Sucht automatisch
// das groesste sichtbare Fenster (ausser der eigenen Konsole).
#include <windows.h>
#include <dwmapi.h>
#include <d3d11.h>
#include <dxgi.h>
...
struct FindCtx { HWND best = nullptr; long bestArea = 0; HWND self = nullptr; };
...
int main() {
    ...
    FindCtx fc; fc.self = GetConsoleWindow();
    EnumWindows(enumProc, (LPARAM)&fc);
    if (!fc.best) { printf("FEHLER: kein geeignetes Fenster gefunden\n"); return 1; }
    wchar_t title[256] = {}; GetWindowTextW(fc.best, title, 255);
    printf("Fenster gewaehlt: \"%ls\"\n", title);
    ...
    // CreateForWindow statt CreateForMonitor - das ist der einzige Unterschied zum Monitor-Weg
    ...
    winrt::check_hresult(interop->CreateForWindow(fc.best,
        winrt::guid_of<wg::GraphicsCaptureItem>(), winrt::put_abi(item)));
    auto sz = item.Size();
    int W = sz.Width & ~1, H = sz.Height & ~1;
    if (W < 2 || H < 2) { printf("FEHLER: Fenstergroesse %dx%d\n", W, H); return 1; }
    printf("Fenster-Groesse: %dx%d\n", W, H);
    ...
    printf("WGC-Fenster-Session gestartet.\n");
    ...
    printf("NVENC bereit fuer das Fenster.\n");

    FILE* f = nullptr; fopen_s(&f, "poc_window.h264", "wb");
    ...
    while (got < 90 && tries < 1500) {
        ...
        if (got % 30 == 0 && got) printf("  %d Fenster-Frames encodiert...\n", got);
    }
    ...
    printf("OK: %d Fenster-Frames encodiert, poc_window.h264 = %zu Bytes.\n", got, totalBytes);
