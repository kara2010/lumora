...
static const int PID_VIDEO = 0x0100, PID_AUDIO = 0x0101;
...
void TsDemux::packet(const uint8_t* p) {
    ...
    if (pid != PID_VIDEO && pid != PID_AUDIO) return;      // PAT/PMT/Null: uninteressant (Dialekt ist fest)
    ...
    bool video = (pid == PID_VIDEO);
    int& lastCc = video ? ccVideo_ : ccAudio_;
    if (lastCc >= 0 && cc != ((lastCc + 1) & 0x0F)) ++ccErrors;  // Zaehlen, nicht verwerfen (PES-Laenge/AUD begrenzen Schaden)
    lastCc = cc;

    std::vector<uint8_t>& pes = video ? pesVideo_ : pesAudio_;
