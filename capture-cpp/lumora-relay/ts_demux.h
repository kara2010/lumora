...
//   optional Video PID 0x102 (AV1 Temporal Units, low-overhead OBU, PES 0xE0, PTS),
...
struct TsDemux {
    // Callbacks: Annex-B-Access-Unit (inkl. AUD) mit 90kHz-PTS / AV1-Temporal-Unit mit
    // 90kHz-PTS / rohes Opus-Paket mit 90kHz-PTS
    std::function<void(const uint8_t* au, size_t len, uint64_t pts90k)> onVideoAU;
    std::function<void(const uint8_t* tu, size_t len, uint64_t pts90k)> onVideoAuAv1;
    ...
    uint64_t videoAUs = 0, videoAUsAv1 = 0, audioFrames = 0, tsPackets = 0, ccErrors = 0;
...
private:
    ...
    std::vector<uint8_t> pesVideo_, pesVideoAv1_, pesAudio_;
    int ccVideo_ = -1, ccVideoAv1_ = -1, ccAudio_ = -1;
};
