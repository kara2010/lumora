...
// Router-Portfreigabe-Anleitung. Erscheint, wenn die automatische UPnP-Freigabe
// beim Streamstart scheitert. Die Texte sind pro Router als vollstaendige, nummerierte
// Schritt-fuer-Schritt-Anleitung (mehrzeilig, \n) hinterlegt und je nach App-Sprache
// direkt DE/EN – KEIN i18n-Key-Lookup noetig (der Renderer zeigt steps 1:1 an).
// Menuepfade an offiziellen Hersteller-/Provider-Quellen verifiziert (2026-07).
// Der KERN jeder Anleitung: erst den PC als Geraet auswaehlen/hinzufuegen (viele
// scheitern genau daran), dann BEIDE Ports (TCP + UDP) freigeben; feste IP empfohlen.
function routerUpnpHint() {
  ...
  const de = mainLang() !== 'en'
  const T = BROADCAST_PORT, U = MTX_ICE_UDP
  // [regex, brand, de-Anleitung, en-Anleitung]
  const brands = [
    [/avm|fritz|1 ?und ?1|1 ?& ?1|homeserver/, 'FRITZ!Box',
...
Tipp: Hakst du beim PC-Eintrag „Selbstständige Portfreigaben für dieses Gerät erlauben“ an, öffnet Lumora den Port künftig selbst.`,
`Open fritz.box in your browser and sign in.
...
Tip: If you tick “Allow independent port sharing for this device” on the PC entry, Lumora can open the port itself next time.`],

    [/speedport|telekom/, 'Speedport',
`Öffne speedport.ip (oder 192.168.2.1) und melde dich mit dem Gerätepasswort (Router-Rückseite) an. Speedport-Router können KEIN UPnP – die manuelle Freigabe ist hier der einzige Weg.
...
6. Über „Weitere Port-Weiterleitung“ eine zweite Regel mit Port ${U} anlegen. Speichern.`,
`Open speedport.ip (or 192.168.2.1) and sign in with the device password (on the back of the router). Speedport routers do NOT support UPnP – the manual steps are the only way here.
...
6. Use “Add further port forwarding” to create a second rule with port ${U}. Save.`],

    [/vodafone/, 'Vodafone Station',
`Öffne 192.168.0.1, klicke oben rechts auf das Menü-Symbol und melde dich an (Passwort auf dem Aufkleber).
...
Hinweis: Fehlt der Menüpunkt „Port-Mapping“, hast du einen DS-Lite-Anschluss (keine öffentliche IPv4) – dann ist keine Freigabe möglich; nutze den IPv6-Weg (unten) oder beantrage bei Vodafone „Dual Stack“.`,
`Open 192.168.0.1, click the menu icon at the top right and sign in (password on the sticker).
...
Note: If there's no “Port-Mapping” menu, you have a DS-Lite line (no public IPv4) – then forwarding is impossible; use the IPv6 path (below) or ask Vodafone for “Dual Stack”.`],

    [/\bo2\b|telefonica|homebox|o2 ?box/, 'o2 HomeBox',
`Öffne o2.box (oder 192.168.1.1) und melde dich an (Passwort auf der Geräterückseite).
...
Hinweis: Bei einem DS-Lite-Anschluss ist keine IPv4-Freigabe möglich – dann den IPv6-Weg (unten) nutzen. (o2-Kabel liefert oft FRITZ!Box-Modelle – dann gilt die FRITZ!Box-Anleitung.)`,
`Open o2.box (or 192.168.1.1) and sign in (password on the back of the device).
...
Note: On a DS-Lite line no IPv4 forwarding is possible – use the IPv6 path (below). (o2 cable often ships FRITZ!Box models – then use the FRITZ!Box guide.)`],

    [/tp-?link|archer/, 'TP-Link',
`Öffne tplinkwifi.net (oder 192.168.0.1) im Browser und melde dich an.
...
Tipp: Gib dem PC vorher eine feste IP (Address Reservation), sonst wandert die Regel bei IP-Wechsel ins Leere. UPnP-Alternative: „Advanced › NAT Forwarding › UPnP“ einschalten – dann macht Lumora es selbst.`,
`Open tplinkwifi.net (or 192.168.0.1) in your browser and sign in.
...
Tip: Give the PC a fixed IP first (Address Reservation), otherwise the rule breaks when the IP changes. UPnP alternative: enable “Advanced › NAT Forwarding › UPnP” – then Lumora does it itself.`],

    [/asus(tek)?/, 'ASUS',
`Öffne router.asus.com (oder 192.168.1.1) im Browser und melde dich an.
...
Tipp: Gib dem PC vorher per DHCP-Reservierung eine feste IP. UPnP-Alternative: „WAN › Internet Connection › Enable UPnP: Yes“.`,
`Open router.asus.com (or 192.168.1.1) in your browser and sign in.
...
Tip: Give the PC a fixed IP via DHCP reservation first. UPnP alternative: “WAN › Internet Connection › Enable UPnP: Yes”.`],

    [/netgear|nighthawk|\borbi\b/, 'Netgear',
`Öffne routerlogin.net im Browser, Benutzer „admin“ und dein Passwort.
...
Tipp: Gib dem PC vorher eine reservierte IP (Address Reservation). UPnP-Alternative: „ADVANCED › Advanced Setup › UPnP › Turn UPnP On“.`,
`Open routerlogin.net in your browser, user “admin” and your password.
...
Tip: Give the PC a reserved IP first (Address Reservation). UPnP alternative: “ADVANCED › Advanced Setup › UPnP › Turn UPnP On”.`],

    [/d-?link/, 'D-Link',
`Öffne dlinkrouter.local (oder 192.168.0.1) im Browser und melde dich an.
...
Tipp: Gib dem PC vorher eine feste IP (DHCP-Reservierung).`,
`Open dlinkrouter.local (or 192.168.0.1) in your browser and sign in.
...
Tip: Give the PC a fixed IP first (DHCP reservation).`],

    [/linksys|belkin/, 'Linksys',
`Öffne LinksysSmartWiFi.com (oder 192.168.1.1); lokal über „click here“ mit dem Router-Passwort anmelden.
...
Wichtig: Linksys verlangt eine feste IP – gib dem PC vorher eine statische/reservierte IP.`,
`Open LinksysSmartWiFi.com (or 192.168.1.1); sign in locally via “click here” with the router password.
...
Important: Linksys requires a fixed IP – give the PC a static/reserved IP first.`],

    [/huawei/, 'Huawei',
`Öffne 192.168.3.1 im Browser und melde dich an (Passwort auf dem Typenschild).
...
Hinweis: UPnP ist ab Werk AUS. Möchtest du, dass Lumora es selbst macht, aktiviere „More Functions › Network Settings › UPnP“.`,
`Open 192.168.3.1 in your browser and sign in (password on the label).
...
Note: UPnP is off by default. To let Lumora do it itself, enable “More Functions › Network Settings › UPnP”.`],

    [/bt\s?smart\s?hub|bthomehub|\bbt\s?hub\b/, 'BT Smart Hub',
`Öffne 192.168.1.254 im Browser (Admin-Passwort auf der Hub-Karte / Rückseite).
...
Tipp: Zur Sicherheit die IP reservieren unter „Advanced settings › My Network“ › Gerät › „Always use this IP address = YES“.`,
`Open 192.168.1.254 in your browser (admin password on the hub card / back).
...
Tip: To be safe, reserve the IP under “Advanced settings › My Network” › device › “Always use this IP address = YES”.`],

    [/sky\s?hub|sky\s?router|sky\s?broadband/, 'Sky Router',
`Sky hat zwei Hub-Typen – der Weg unterscheidet sich:
...
WEISSER WiFi-Max-Hub (SR213): geht nur in der „My Sky“-App › Broadband › Product Settings › Advanced settings › Gerät wählen › Ports ${T} (TCP) und ${U} (UDP) eintragen › Save.`,
`Sky has two hub types – the path differs:
...
WHITE WiFi Max hub (SR213): only via the “My Sky” app › Broadband › Product Settings › Advanced settings › select the device › enter ports ${T} (TCP) and ${U} (UDP) › Save.`],

    [/virgin\s?media|super\s?hub/, 'Virgin Media Hub',
`Wichtig: Der Hub muss im „Router mode“ laufen – im „Modem mode“ ist Portfreigabe abgeschaltet.
...
5. Speichern (der Hub verlangt evtl. einen Neustart).`,
`Important: The hub must run in “Router mode” – in “Modem mode” port forwarding is disabled.
...
5. Save (the hub may need a reboot).`],

    [/xfinity|xfi\s?gateway|comcast/, 'Xfinity Gateway',
`Bei Xfinity xFi geht Portfreigabe NUR über die Xfinity-App (nicht im Web-Menü).
...
Falls es trotz Regel nicht klappt: in der App unter „Advanced Security“ für diesen PC „Allow Access“ setzen.`,
`On Xfinity xFi, port forwarding works ONLY via the Xfinity app (not the web menu).
...
If it still won't work despite the rule: in the app under “Advanced Security”, set “Allow Access” for this PC.`],

    [/\bbgw\d|at&t|arris.?bgw/, 'AT&T Gateway',
`AT&T-Gateways bieten kein UPnP – die manuelle Freigabe ist hier Pflicht.
...
4. „Return to NAT/Gaming“ › bei „Needed by Device“ diesen PC wählen › „Add“ › „Save“.`,
`AT&T gateways don't offer UPnP – the manual steps are mandatory here.
...
4. “Return to NAT/Gaming” › under “Needed by Device” select this PC › “Add” › “Save”.`],
  ]
  for (const [re, brand, dS, eS] of brands) {
    if (re.test(nm)) return { router: name || brand, steps: de ? dS : eS }
  }
  // Marke unklar, aber ein bekannter Provider-Chipsatz erkannt: allgemeingueltiges Muster.
  const genericDe =
`Öffne den Router-Adminbereich im Browser (meist 192.168.0.1 oder 192.168.1.1; das Passwort steht oft auf dem Router). Anbieter-Router lassen sich manchmal nur über die App/das Kundenportal des Anbieters einstellen.
...
Alternativ „UPnP“ / „selbstständige Portfreigaben“ im Router aktivieren – dann öffnet Lumora den Port selbst.`
  const genericEn =
`Open the router admin page in your browser (usually 192.168.0.1 or 192.168.1.1; the password is often printed on the router). Provider routers can sometimes only be changed via the provider's app or customer portal.
...
Alternatively enable “UPnP” / “automatic port forwarding” in the router – then Lumora opens the port itself.`
  if (/arris|technicolor|hitron|sagemcom|compal|askey|zyxel|\bzte\b/.test(nm)) {
    return { router: name || (de ? 'deinem Anbieter-Router' : 'your provider router'), steps: de ? genericDe : genericEn }
  }
  return { router: name || (de ? 'deinem Router' : 'your router'), steps: de ? genericDe : genericEn }
}
...
async function runConnectivityTest() {
  ...
  if (tcpOk && udpOk) {
    ...
    steps.push({ key: 'upnp', state: 'warn', label: 'Router-Portfreigabe (UPnP)', detail: 'Der Router öffnet die IPv4-Ports nicht selbst. ' + found + 'Die genaue Schritt-für-Schritt-Anleitung für deinen Router bekommst du beim Streamstart im Stream-Tab.' + alt })
  }
